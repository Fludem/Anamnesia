...
Phase 16 is Sorcery: a ninth listed skill that is also the other way to fight. At the bench it
inscribes marks (the tier's ore pressed with ash — firemaking's ash finally has a buyer) and
staffs (the tier's log shod with its bar); in the zones a staff casts on the Sorcery level and
pays Sorcery, burning one mark from the ammo slot per landed cast the way a javelin is thrown —
except that without marks the staff is a stick and the fight will not start. The worn weapon
decides the fight, not a toggle; ammo of the other style is carried and nothing more. Every
monster is weak to one style and takes a quarter more from it, so each zone has work for a
sword and a staff; `scripts/tune-combat.ts` now sets hp against the style a monster is for and
